function merge_error_saved_bysessionnumber(sname)
...
%% 
% Copied from merge_error_saved, modified for data after JK030 (starting from JK036, 11/28/2017) because of saved.SavingSection_SessionName contains the information about session number. Capable of doing multiple different sessions in a day.
% Merge only when session names are the same. 
% Session name format: S## 
% Result data format: 'data_JK###_S##.mat'
% 2018/02/13 JK

if isnumeric(sname) && sname < 100
    sname = sprintf('S%02d',sname);
end
dirpath = strsplit(pwd,'\');
mname = dirpath{end};
...
if isempty(list)
    fprintf('no matching session name in %s of %s\n',sname,mname);
    return
end
