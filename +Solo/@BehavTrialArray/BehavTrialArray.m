...
% DHO, 5/08.
%
% STILL MUST IMPLEMENT JOINING SOLO FILES!!
%
%

classdef BehavTrialArray < handle
    ...
    methods (Access = public)
        function obj = BehavTrialArray(x, session_name)
            ...
            if nargin > 0
                ...
                n=1;
                savedfields = fieldnames(x.saved);
...
%         end
        ...
        function plot_scored_trials_stim(obj,stimTrials, varargin)
            ...
            if ~isempty(cr_stim)
                plot(cr_stim,ones(size(cr_stim))-.1, 'b*', 'MarkerSize',ms); hold on
            end
