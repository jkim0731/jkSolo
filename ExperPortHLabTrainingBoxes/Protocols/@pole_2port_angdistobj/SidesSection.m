...
function [x, y] = SidesSection(obj, action, x, y)
   ...
   switch action
    ...
    case 'init',   % ------------ CASE INIT ----------------
      ...
      SoloFunctionAddVars('RewardsSection', 'ro_args', 'previous_sides');
      ...
      NumeditParam(obj, 'CatchProb', 0, x, y); 
      next_row(y, 1);
      ...
      set(value(myaxes), 'YTick', [1 2 3 4 5], 'YLim', [0.5 5.5], 'YTickLabel', ...
                        {'R-dR', 'R-dL', 'No-go', 'L-dR', 'L-dL'});
    ...
    case 'choose_next_side', % --------- CASE CHOOSE_NEXT_SIDE -----
        ...
        elseif strcmp(Distractor, 'Discrete')
            next_dstr = 'a';        
        else next_dstr = 'n'; % no distractor
        end
        ...
        if strcmp(Catch, 'On')
            if rand < CatchProb
                next_side = 'o'; % o for OUT (of reach)
                next_dstr = 'o'; % o for OUT (of reach)
            end
        end
        ...
        if ~isempty(next_dstr)
            ...
            next_dstr = 'n'; % another safety, making next_dstr to none when it is not define
      ...
      if strcmp(Catch, 'On')
          switch Distractor
              case 'Off'
                  ...
                  if ps(end)=='l'
                     hb = line(length(previous_sides), 5, 'Parent', value(myaxes));
                  elseif ps(end) == 'r'                 
                     hb = line(length(previous_sides), 1, 'Parent', value(myaxes));
                  elseif ps(end) == 'o' % catch trials
                  ...
                  nogos  = find(previous_sides(xgreen) == 'o');
                  ygreen = zeros(size(xgreen)); ygreen(lefts) = 5; ygreen(rghts) = 1; ygreen(nogos) = 3;
                  ...
                  nogos = find(previous_sides(xred) == 'o');
                  yred = zeros(size(xred)); yred(lefts) = 5; yred(rghts) = 1; yred(nogos) = 3;
                  ...
                  yblack = zeros(size(xblack)); yblack(lefts) = 5; yblack(rghts) = 1;
                ...
                case 'On'
                  ...
                  elseif ps(end) == 'o' % for catch trials, treated as no-go
                      hb = line(length(previous_sides), 3, 'Parent', value(myaxes));
                  end;
                  set(hb, 'Color', 'b', 'Marker', '.', 'LineStyle', 'none');
                  ...
                  nogos = find(previous_dstrs(xgreen) == 'o');
                  ygreen = zeros(size(xgreen)); 
                  ygreen(intersect(lefts,dls)) = 5; 
                  ygreen(intersect(lefts,drs)) = 4;
                  ygreen(nogos) = 3;
                  ...
                  nogos = find(previous_dstrs(xred) == 'o');
                  yred = zeros(size(xred)); 
                  yred(intersect(lefts,dls)) = 5; 
                  yred(intersect(lefts,drs)) = 4;
                  yred(nogos) = 3;
                  ...
                  yblack(intersect(lefts,dls)) = 5; 
                  yblack(intersect(lefts,drs)) = 4;
              ...
              case 'Continuous' % same as in case of 'Off', except now they are in the middle of the rows
                  ...
                  rghts  = find(previous_sides(xgreen) == 'r');
                  nogos  = find(previous_sides(xgreen) == 'o');
                  ygreen = zeros(size(xgreen)); ygreen(lefts) = 4; ygreen(rghts) = 2; ygreen(nogos) = 3;
                  hg = line(xgreen, ygreen, 'Parent', value(myaxes));
                  set(hg, 'Color', 'g', 'Marker', '.', 'LineStyle', 'none'); 
                  ...
                  rghts = find(previous_sides(xred) == 'r');
                  nogos  = find(previous_sides(xred) == 'o');
                  yred = zeros(size(xred)); yred(lefts) = 4; yred(rghts) = 2; yred(nogos) = 3;
                  hr = line(xred, yred, 'Parent', value(myaxes));
                  set(hr, 'Color', 'r', 'Marker', '.', 'LineStyle', 'none'); 
                  ...
                  lefts = find(previous_sides(xblack) == 'l');
                  rghts = find(previous_sides(xblack) == 'r');
                  yblack = zeros(size(xblack)); yblack(lefts) = 4; yblack(rghts) = 2;
                  hk = line(xblack, yblack, 'Parent', value(myaxes));
                  set(hk, 'Color', 'k', 'Marker', 'x', 'LineStyle', 'none'); 
          ...
          switch Distractor
              case 'Off'
                  ...
                  lefts  = find(previous_sides(xgreen) == 'l');
                  rghts  = find(previous_sides(xgreen) == 'r');
                  ygreen = zeros(size(xgreen)); ygreen(lefts) = 5; ygreen(rghts) = 1;
                  hg = line(xgreen, ygreen, 'Parent', value(myaxes));
                  set(hg, 'Color', 'g', 'Marker', '.', 'LineStyle', 'none'); 
                  ...
                  lefts = find(previous_sides(xred) == 'l');
                  rghts = find(previous_sides(xred) == 'r');
                  yred = zeros(size(xred)); yred(lefts) = 5; yred(rghts) = 1;
                  hr = line(xred, yred, 'Parent', value(myaxes));
                  set(hr, 'Color', 'r', 'Marker', '.', 'LineStyle', 'none'); 
              ...
              case 'Continuous' % same as in case of 'Off', except now they are in the middle of the rows
                  ...
                  lefts  = find(previous_sides(xgreen) == 'l');
                  rghts  = find(previous_sides(xgreen) == 'r');
                  ygreen = zeros(size(xgreen)); ygreen(lefts) = 4; ygreen(rghts) = 2;
                  hg = line(xgreen, ygreen, 'Parent', value(myaxes));
                  set(hg, 'Color', 'g', 'Marker', '.', 'LineStyle', 'none'); 
                  ...
                  lefts = find(previous_sides(xred) == 'l');
                  rghts = find(previous_sides(xred) == 'r');
                  yred = zeros(size(xred)); yred(lefts) = 4; yred(rghts) = 2;
                  hr = line(xred, yred, 'Parent', value(myaxes));
                  set(hr, 'Color', 'r', 'Marker', '.', 'LineStyle', 'none'); 
