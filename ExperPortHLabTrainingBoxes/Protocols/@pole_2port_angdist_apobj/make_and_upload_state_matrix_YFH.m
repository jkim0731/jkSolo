...
%            end
           ...
           m(end,5) = 201; % jump back to PREPAUSE.
           
           stm = [stm; zeros(101-rows(stm),8)];
           stm = [stm; m];
           

           %------ Microstepper rotation direction on digital output given by 'slid':
           % Requires that states 201 through 201+2*R_numbits be reserved
           % for giving bit signal.
